  ...
  module React
    class ReactGenerator
      ...
      def generate_component_file(name, jsx_content, json)
        ...
        # Also check if component uses @{props.xxx} bindings - if so, add 'props' parameter
        uses_props_bindings = uses_props_bindings?(json)
        ...
        props_sig = if uses_props_bindings
                      'props'
                    else
                      generate_props_signature(props)
                    end

        <<~JSX
          #{use_client}// Generated by ReactJsonUI - Do not edit directly
          #{react_import}#{string_manager_import}#{extension_imports}#{component_imports}
      ...
      # Check if the JSON uses @{props.xxx} bindings
      def uses_props_bindings?(json)
        # Check all string values for @{props.xxx} pattern
        json.each do |_key, value|
          case value
          when String
            return true if value.match?(/@\{props\.[^}]+\}/)
          when Hash
            return true if uses_props_bindings?(value)
          when Array
            value.each do |item|
              return true if item.is_a?(Hash) && uses_props_bindings?(item)
            end
          end
        end
