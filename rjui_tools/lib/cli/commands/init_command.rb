      ...
      class InitCommand
        ...
        def create_string_manager(config)
          ...
          imports = languages.map do |lang|
            "import #{lang}Strings from '../#{strings_dir.sub('src/', '')}/#{lang}.json';"
          end.join("\n")

          # Generate strings object
          strings_entries = languages.map { |lang| "  #{lang}: #{lang}Strings" }.join(",\n")
          ...
          content = <<~JS
            ...
            class StringManagerClass {
              ...
              get currentLanguage() {
                return strings[this._currentLanguage] || strings['#{default_language}'];
              }
